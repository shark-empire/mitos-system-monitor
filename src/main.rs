...
#[derive(PartialEq)]
enum Tab { Overview, Processes, TerminalBuffers }
...
/// Scan the runtime dir, query every mitos-terminal, prune dead sockets.
async fn refresh_terminals(state: &Arc<Mutex<SharedState>>) {
    ...
    for (_pid, path) in ipc::list_terminal_sockets() {
        match query_terminal(&path).await {
            ...
            Err(_) => { let _ = std::fs::remove_file(&path); } // stale socket from a crashed terminal
        }
    }
...
async fn query_terminal(path: &str) -> std::io::Result<TerminalSnapshot> {
    ...
    match ipc::ipc_recv::<IpcResponse>(&mut stream).await? {
        Some(IpcResponse::BufferData { pid, prompt, text }) => Ok(TerminalSnapshot {
            pid, prompt, text, last_seen: Instant::now(),
        }),
        Some(other) => Err(std::io::Error::new(
            std::io::ErrorKind::InvalidData,
            format!("unexpected response: {other:?}"),
        )),
        None => Err(std::io::Error::new(std::io::ErrorKind::UnexpectedEof, "stream closed")),
    }
}
...
impl MitosMonitorApp {
    ...
    fn send_alert(&self, pid: u32, name: &str, cpu: f32) {
        ...
        let button = RichWidget::Button {
            label: format!("⚠️ Kill runaway: {} (PID {}) — {:.0}% CPU", name, pid, cpu),
            cmd: format!("kill -9 {}", pid),
        };

        let _ = self.action_tx.try_send(MonitorAction::Inject { terminal_pid: target, widget: gauge });
        let _ = self.action_tx.try_send(MonitorAction::Inject { terminal_pid: target, widget: button });
    }
}
    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        egui::TopBottomPanel::top("top_bar").show(ctx, |ui| {
            ui.horizontal(|ui| {
                ...
                ui.selectable_value(&mut self.selected_tab, Tab::TerminalBuffers, "📟 Terminal Buffers");
...
impl MitosMonitorApp {
    fn draw_overview(&mut self, ui: &mut egui::Ui) {
        ...
        ui.columns(2, |cols| {
            cols[0].group(|ui| {
                ui.heading("CPU %");
                let pts: Vec<[f64; 2]> = self.cpu_history.iter().enumerate().map(|(i, &v)| [i as f64, v]).collect();
                Plot::new("cpu_plot")
                    .height(200.0)
                    .include_y(0.0)
                    .include_y(100.0)
            ...
            cols[1].group(|ui| {
                ui.heading("Memory %");
                let pts: Vec<[f64; 2]> = self.ram_history.iter().enumerate().map(|(i, &v)| [i as f64, v]).collect();
                Plot::new("ram_plot")
                    .height(200.0)
                    .include_y(0.0)
                    .include_y(100.0)
    ...
    fn draw_processes(&mut self, ui: &mut egui::Ui) {
        ...
        egui::ScrollArea::vertical().show(ui, |ui| {
            egui::Grid::new("proc_grid")
                ...
                .min_col_width(70.0)
                .show(ui, |ui| {
                    ui.strong("PID"); ui.strong("Name"); ui.strong("CPU %"); ui.strong("RAM MB"); ui.strong("Actions");
                    ui.end_row();
    ...
    fn draw_terminal_buffers(&mut self, ui: &mut egui::Ui) {
        ...
        for snap in &g.terminals {
            ...
            ui.group(|ui| {
                ...
                // Global search across this terminal's entire history
                ...
                egui::CollapsingHeader::new(format!("📄 View full buffer ({} lines)", snap.text.lines().count()))
